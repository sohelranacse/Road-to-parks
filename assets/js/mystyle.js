                  ...
                  '<div class="col-md-10 pad0 col-sm-10">'+
                    '<div class="listing-info">'+
                      ...
                      '<div class="col-md-4 col-sm-4 col-xs-4 text-right">'+
                        '<h4 class="btn btn-default" disable>&#2547;'+amount+'</h4>'+book_now+
                      '</div>'+
                    '</div>'+
                  '</div>'+
              '</div>';
  ...
  geocoder.geocode( {address:address}, function(results, status) 
  {
    if (status == google.maps.GeocoderStatus.OK){
      ...
      var marker = new google.maps.InfoWindow({
        ...
        content: '<a class="map_info_style" href="'+li+'home/host_rv/'+id+'/'+title.split(' ').join('-')+'" target="_blank">&#2547;'+amount+'</a>'
      });
    }
  });
}
...
function handCash_booking(){
  ...
  if (hostid !== '' && m_id !== '' && from_date !== '' && to_date !== '') {
    $.ajax({
      type: 'POST',
      url:li+'user/booking_handcash',
      data:{hostid:hostid,m_id:m_id,from_date:from_date,to_date:to_date},
      dataType:'json',
      success: function(response){
